...
(function() {
    ...
    var generateMotionAABB = function() {
        for (var i = 0, rb; rb = objects[i++];) {
            ...
            // save bounding box
            rb.motionBounds = [
                (min[0] + max[0]) * 0.5, (min[1] + max[1]) * 0.5, (max[0] - min[0]) * 0.5, (max[1] - min[1]) * 0.5
            ];
        }
    }
    ...
    var solve = function() {
        for (var j = 0; j < numIterations; j++) {
            for (var i = 0; i < contactsI; i++) {
                ...
                var dv = [
                    (b.vel[0] + rb[0] * b.angularVel) - (a.vel[0] + ra[0] * a.angularVel), (b.vel[1] + rb[1] * b.angularVel) - (a.vel[1] + ra[1] * a.angularVel)
                ];
    ...
    var resize = function() {
        ...
        objects.push(new Rectangle(false, scr.width * 0.5, scr.height, scr.width * 1, 1, 0, 0)); // floor
    }
    var drop = function() {
            for (var i = 0; i < 10; i++) {
                dropc++;
